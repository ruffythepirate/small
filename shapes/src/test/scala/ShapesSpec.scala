import SolveShapes.Coord
import org.scalatest.FunSpec

class ShapesSpec extends FunSpec {
  ...
  describe("Find non empty") {
    ...
    taken2x2.foreach(c => {
      it(s"Verify that $c is taken for 2x2") {
        ...
        val notEmpty = SolveShapes.findNonEmpty(0, 0, 2, 2, board)
    ...
    taken4x4.foreach(c => {
      it(s"Verify that $c is taken for 4x4") {
        ...
        val notEmpty = SolveShapes.findNonEmpty(0, 0, 4, 4, board)
  ...
  describe("Placing walls 2x2") {
    ...
    testData.foreach(td => {
      it(s"Can place walls in 2x2 case taken ${td._1}") {
        ...
        val board = Array.ofDim[Int](2, 2)
        board(c.x)(c.y) = 1
        SolveShapes.populateGrid(0, 0, 2, 2, board)
  ...
  describe("Placing walls 4x4") {
    ...
    testData.foreach(td => {
      it(s"Can place walls in 4x4 case taken ${td._1}") {
        ...
        val board = Array.ofDim[Int](4, 4)
        board(c.x)(c.y) = 1
        SolveShapes.populateGrid(0, 0, 4, 4, board)
